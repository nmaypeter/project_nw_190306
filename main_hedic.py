                ...
                for prod_setting2 in prod_setting2_seq:
                    ...
                    for bud in range(begin_budget, total_budget + 1):
                        ...
                        for sample_count in range(sample_number):
                            print('@ mhedic seed selection @ data_set_name = ' + data_set_name + ', wpiwp = ' + str(wpiwp) +
                                  ', product_name = ' + product_name + ', budget = ' + str(bud) + ', sample_count = ' + str(sample_count))
                            now_budget = 0.0
                            seed_set = [set() for _ in range(num_product)]

                            degree_dict = sshd_main.constructExpendDegreeDict()
                        ...
                        for distribution_type in dis_sequence:
                            for pps in pps_seq:
                                ...
                                for sample_count, sample_seed_set in enumerate(seed_set_sequence):
                                    print('@ mhedic evaluation @ data_set_name = ' + data_set_name + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                ...
                                path1 = 'result/mhedic_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp
                                if not os.path.isdir(path1):
                                    os.mkdir(path1)
                                path = 'result/mhedic_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp + '/' + data_set_name + '_' + product_name
                                if not os.path.isdir(path):
                                    os.mkdir(path)
                                fw = open(path + '/' + 'b' + str(bud) + '_i' + str(sample_number) + '.txt', 'w')
                                fw.write('mhedic, pp_strategy = ' + str(pps) + ', total_budget = ' + str(bud) + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) + '\n' +
