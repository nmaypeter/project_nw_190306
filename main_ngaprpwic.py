            ...
            for data_setting in data_setting_seq:
                ...
                for prod_setting in prod_setting_seq:
                    for prod_setting2 in prod_setting2_seq:
                        ...
                        ssngaprpw_main = SeedSelectionNGAPPW(graph_dict, seed_cost_dict, product_list, distribution_type)
                        p_w_list = ssngaprpw_main.getProductWeight()
                        diffappw_main = DiffusionAccProbPW(graph_dict, seed_cost_dict, product_list, p_w_list)
                        for sample_count in range(sample_number):
                            ss_strat_time = time.time()
                            begin_budget = 1
                            now_budget, now_profit = 0.0, 0.0
                            seed_set = [set() for _ in range(num_product)]
                            celf_sequence = ssngaprpw_main.generateCelfSequenceR(p_w_list)
