            ...
            for data_setting in data_setting_seq:
                ...
                for prod_setting in prod_setting_seq:
                    for prod_setting2 in prod_setting2_seq:
                        ...
                        sshdpw_main = SeedSelectionHDPW(graph_dict, seed_cost_dict, product_list, distribution_type)
                        pw_list = sshdpw_main.getProductWeight()
                        for sample_count in range(sample_number):
                            ss_strat_time = time.time()
                            begin_budget = 1
                            now_budget = 0.0
                            seed_set = [set() for _ in range(num_product)]
                            degree_dict = sshdpw_main.constructDegreeDict(data_set_name, pw_list)
                            ...
                            while len(temp_sequence) != 0:
                                ...
                                print('@ mhdpwic seed selection @ data_set_name = ' + data_set_name + ', wpiwp = ' + str(wpiwp) +
                                      ', product_name = ' + product_name + ', budget = ' + str(begin_budget) + ', sample_count = ' + str(sample_count))
                                mep_g, degree_dict = sshdpw_main.getHighDegreeNode(degree_dict)
                        ...
                        for bud in range(1, total_budget + 1):
                            ...
                            for pps in pps_seq:
                                ...
                                for sample_count, sample_seed_set in enumerate(seed_set_sequence[bud - 1]):
                                    print('@ mhdpwic evaluation @ data_set_name = ' + data_set_name + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                ...
                                if not os.path.isdir(path1):
                                    os.mkdir(path1)
                                path = 'result/mhdpwic_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp + '/' + data_set_name + '_' + product_name
