                ...
                for prod_setting2 in prod_setting2_seq:
                    ...
                    for distribution_type in dis_sequence:
                        for bud in range(begin_budget, total_budget + 1):
                            ...
                            sshd_main = SeedSelectionHDPW(graph_dict, seed_cost_dict, product_list, bud, distribution_type)
                            pw_list = sshd_main.getProductWeight()
                            seed_set_sequence = []

                            for sample_count in range(sample_number):
                                print('@ mhedpwic seed selection @ data_set_name = ' + data_set_name + ', wpiwp = ' + str(wpiwp) +
                                      ', product_name = ' + product_name + ', budget = ' + str(bud) + ', sample_count = ' + str(sample_count))
                                now_budget = 0.0
                                seed_set = [set() for _ in range(num_product)]

                                degree_dict = sshd_main.constructExpendDegreeDict(pw_list)
                                ...
                                del degree_dict, mep_k_prod, mep_i_node
                            ...
                            for pps in pps_seq:
                                ...
                                for sample_count, sample_seed_set in enumerate(seed_set_sequence):
                                    print('@ mhedpwic evaluation @ data_set_name = ' + data_set_name + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                ...
                                if not os.path.isdir(path1):
                                    os.mkdir(path1)
                                path = 'result/mhedpwic_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp + '/' + data_set_name + '_' + product_name
