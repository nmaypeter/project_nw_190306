                    ...
                    for prod_setting in prod_setting_seq:
                        for prod_setting2 in prod_setting2_seq:
                            ...
                            for sample_count in range(sample_number):
                                ...
                                celf_sequence = ssngap_main.generateCelfSequenceR()
                                ...
                                while len(temp_sequence) != 0:
                                    ...
                                    print('@ mngapsric seed selection @ data_set_name = ' + data_set_name + '_' + cas_model + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                            ...
                            for bud in range(1, total_budget + 1):
                                ...
                                for pps in pps_seq:
                                    ...
                                    for sample_count, sample_seed_set in enumerate(seed_set_sequence[bud - 1]):
                                        print('@ mngapsric evaluation @ data_set_name = ' + data_set_name + '_' + cas_model + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                    ...
                                    path1 = 'result/mngapsric_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp
                                    if not os.path.isdir(path1):
                                        os.mkdir(path1)
                                    path = 'result/mngapsric_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp + '/' + data_set_name + '_' + cas_model + '_' + product_name
                                    if not os.path.isdir(path):
                                        os.mkdir(path)
                                    fw = open(path + '/b' + str(bud) + '_i' + str(sample_number) + '.txt', 'w')
                                    fw.write('mngapsric, pp_strategy = ' + str(pps) + ', total_budget = ' + str(bud) + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) + '\n' +
