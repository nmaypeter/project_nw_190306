...
class SeedSelectionNGAP:
    ...
    def generateCelfSequence(self):
        ...
        diffap_ss = DiffusionAccProb2(self.graph_dict, self.seed_cost_dict, self.product_list)

        for i in set(self.graph_dict.keys()):
            s_set = [set() for _ in range(self.num_product)]
            s_i_dict = [{} for _ in range(self.num_product)]
            ep, s_i_dict = diffap_ss.getExpectedProfit(0, i, s_set, s_i_dict)
            mg = round(ep, 4)
            ...
            for k in range(self.num_product):
                ...
                s_i_dict[0], s_i_dict[k] = s_i_dict[k], s_i_dict[0]
                celf_ep = [k, i, mg, 0, s_i_dict]
