            ...
            for data_setting in data_setting_seq:
                ...
                for prod_setting in prod_setting_seq:
                    for prod_setting2 in prod_setting2_seq:
                        ...
                        ssngappw_main = SeedSelectionNGAPPW(graph_dict, seed_cost_dict, product_list, distribution_type)
                        pw_list = ssngappw_main.getProductWeight()
                        diffappw_main = DiffusionAccProbPW(graph_dict, seed_cost_dict, product_list, pw_list)
                        for sample_count in range(sample_number):
                            ss_strat_time = time.time()
                            begin_budget = 1
                            now_budget, now_profit = 0.0, 0.0
                            seed_set = [set() for _ in range(num_product)]
                            celf_sequence = ssngappw_main.generateCelfSequence(pw_list)
                            ...
                            while len(temp_sequence) != 0:
                                ...
                                print('@ mngappwic seed selection @ data_set_name = ' + data_set_name + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                ...
                                while now_budget < begin_budget and mep_i_node != '-1':
                                    ...
                                    if mep_flag == seed_set_length:
                                        ...
                                        now_profit = diffappw_main.getSeedSetProfit(seed_set)
                                        now_budget = round(now_budget + sc, 2)
                                    else:
                                        ep_g = diffappw_main.getExpectedProfit(mep_k_prod, mep_i_node, seed_set)
                        ...
                        for bud in range(1, total_budget + 1):
                            ...
                            for pps in pps_seq:
                                ...
                                for sample_count, sample_seed_set in enumerate(seed_set_sequence[bud - 1]):
                                    print('@ mngappwic evaluation @ data_set_name = ' + data_set_name + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                ...
                                path1 = 'result/mngappwic_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp
                                if not os.path.isdir(path1):
                                    os.mkdir(path1)
                                path = 'result/mngappwic_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp + '/' + data_set_name + '_' + product_name
                                if not os.path.isdir(path):
                                    os.mkdir(path)
                                fw = open(path + '/b' + str(bud) + '_i' + str(sample_number) + '.txt', 'w')
                                fw.write('mngappwic, pp_strategy = ' + str(pps) + ', total_budget = ' + str(bud) + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) + '\n' +
