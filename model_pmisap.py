                ...
                for cm in cm_seq:
                    ...
                    for prod_setting in prod_setting_seq:
                        for prod_setting2 in prod_setting2_seq:
                            ...
                            sspmisap_main = SeedSelectionPMISAP(graph_dict, seed_cost_dict, product_list)
                            ...
                            for sample_count in range(sample_number):
                                ...
                                ss_k_time_sequence = [[[] for _ in range(num_product)] for _ in range(total_budget)]
                                for kk in range(num_product):
                                    ss_strat_time = time.time()
                                    begin_budget = 1
                                    cur_budget, cur_profit = 0.0, 0.0
                                    celf_sequence = sspmisap_main.generateCelfSequence()
                                    ...
                                    while len(temp_sequence) != 0:
                                        ...
                                        print('@ mpmisapic seed selection @ data_set_name = ' + data_set_name + '_' + cas_model + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                              ', product_name = ' + product_name + ', budget = ' + str(begin_budget) + ', sample_count = ' + str(sample_count) + str(kk))

                                        seed_set_pmis = copy.deepcopy(s_matrix[kk][-1])
                                        ...
                                        while cur_budget < begin_budget and mep_i_node != '-1':
                                            ...
                                            if cur_budget + sc >= begin_budget and begin_budget < total_budget and len(temp_sequence) == 0:
                                                ...
                                                temp_celf_sequence[kk].insert(0, mep)
                                            ...
                                            if mep_flag == seed_set_length:
                                                ...
                                                cur_profit = diffap_main.getSeedSetProfit(seed_set_pmis)
                                        ...
                                        c_matrix_sequence[begin_budget - 1][kk] = c_matrix[kk]
                                        ss_k_time_sequence[begin_budget - 1][kk] = round(time.time() - ss_strat_time + ss_acc_time, 2)
                                ...
                                for begin_budget in range(1, total_budget + 1):
                                    ...
                                    ss_time = round(time.time() - ss_strat_time + sum(ss_k_time_sequence[begin_budget - 1]), 2)
                            ...
                            for bud in range(1, total_budget + 1):
                                ...
                                for pps in pps_seq:
                                    ...
                                    for sample_count, sample_seed_set in enumerate(seed_set_sequence[bud - 1]):
                                        print('@ mpmisapic evaluation @ data_set_name = ' + data_set_name + '_' + cas_model + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                    ...
                                    path1 = 'result/mpmisapic_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp
                                    if not os.path.isdir(path1):
                                        os.mkdir(path1)
                                    path = 'result/mpmisapic_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp + '/' + data_set_name + '_' + cas_model + '_' + product_name
                                    if not os.path.isdir(path):
                                        os.mkdir(path)
                                    fw = open(path + '/b' + str(bud) + '_i' + str(sample_number) + '.txt', 'w')
                                    fw.write('mpmisapic, pp_strategy = ' + str(pps) + ', total_budget = ' + str(bud) + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) + '\n' +
