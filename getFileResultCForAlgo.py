                        ...
                        for wpiwp in wpiwp_seq:
                            for ms in model_seq:
                                ...
                                if model_seq.index(ms) == 0:
                                    fw = open('result/r_' + data_set_name + '_' + cas_model + '/dis' + str(dis) + '_pps' + str(pps) + '_comparison_profit.txt', 'w')
                                elif model_seq.index(ms) == 1:
                                    fw = open('result/r_' + data_set_name + '_' + cas_model + '/dis' + str(dis) + '_pps' + str(pps) + '_comparison_ng_profit.txt', 'w')
                                elif model_seq.index(ms) == 2:
                                    fw = open('result/r_' + data_set_name + '_' + cas_model + '/dis' + str(dis) + '_pps' + str(pps) + '_comparison_ap_profit.txt', 'w')
                                elif model_seq.index(ms) == 3:
                                    fw = open('result/r_' + data_set_name + '_' + cas_model + '/dis' + str(dis) + '_pps' + str(pps) + '_comparison_hd_profit.txt', 'w')
                                for lnum, line in enumerate(profit):
                                    if lnum % (len(ms) * 2) == 0 and lnum != 0:
                                        fw.write('\n' * 9)
                                    fw.write(str(line) + '\n')
                                fw.close()
