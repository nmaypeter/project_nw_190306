                ...
                for cm in cm_seq:
                    ...
                    for prod_setting in prod_setting_seq:
                        for prod_setting2 in prod_setting2_seq:
                            ...
                            ssr_main = SeedSelectionR(graph_dict, seed_cost_dict, product_list)
                            for sample_count in range(sample_number):
                                ...
                                temp_sequence = [[begin_budget, now_budget, copy.deepcopy(seed_set), copy.deepcopy(random_node_set), ss_acc_time]]
                                while len(temp_sequence) != 0:
                                    ss_strat_time = time.time()
                                    begin_budget, now_budget, seed_set, random_node_set, ss_acc_time = temp_sequence.pop(0)
                                    print('@ mric seed selection @ data_set_name = ' + data_set_name + '_' + cas_model + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                          ', product_name = ' + product_name + ', budget = ' + str(begin_budget) + ', sample_count = ' + str(sample_count))
                                    mep_g, random_node_set = ssr_main.selectRandomSeed(random_node_set)
                                    ...
                                    while now_budget < begin_budget and mep_i_node != '-1':
                                        ...
                                        if now_budget + sc >= begin_budget and begin_budget < total_budget and len(temp_sequence) == 0:
                                            ...
                                            temp_sequence.append([begin_budget + 1, now_budget, copy.deepcopy(seed_set), copy.deepcopy(temp_random_node_set), ss_time])
                            ...
                            for bud in range(1, total_budget + 1):
                                ...
                                for pps in pps_seq:
                                    ...
                                    for sample_count, sample_seed_set in enumerate(seed_set_sequence[bud - 1]):
                                        print('@ mric evaluation @ data_set_name = ' + data_set_name + '_' + cas_model + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) +
                                    ...
                                    path1 = 'result/mric_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp
                                    if not os.path.isdir(path1):
                                        os.mkdir(path1)
                                    path = 'result/mric_pps' + str(pps) + '_dis' + str(distribution_type) + '_wpiwp' * wpiwp + '/' + data_set_name + '_' + cas_model + '_' + product_name
                                    if not os.path.isdir(path):
                                        os.mkdir(path)
                                    fw = open(path + '/b' + str(bud) + '_i' + str(sample_number) + '.txt', 'w')
                                    fw.write('mric, pp_strategy = ' + str(pps) + ', total_budget = ' + str(bud) + ', dis = ' + str(distribution_type) + ', wpiwp = ' + str(wpiwp) + '\n' +
